...
export default {
  ...
  platforms: {
    ...
    claude: {
      ...
      deepLinkPattern: 'https://claude.ai/chat/{uuid}',
      selectors: {
        loginShell:
        ...
        recentsSearch: 'input[placeholder*="Search" i], input[aria-label*="Search" i]',
      },
      endpoints: {
        organizations: '/api/organizations',
        chatConversations: '/api/organizations/{orgId}/chat_conversations',
        projects: '/api/organizations/{orgId}/projects',
        projectConversations: '/api/organizations/{orgId}/projects/{projectId}/conversations',
      },
      pageSize: 20,
      projectListQuery: { limit: 8 },
      notes:
        'Endpoint-first per S2: GET organizations → paginated chat_conversations (client title-match after full-page normalize) → Projects directory + per-project conversations (breadth-first page-1 then deepen, budget-capped). Deep link https://claude.ai/chat/{uuid}. Prefill not supported. Capability title-match. Soft caps: root ~100 / per-project ~60 still authorize empty by design (BL-024). Failure/budget/failed-project truncation → unavailable/timeout not empty (BL-022). DOM Recents search fallback deferred (BL-023). include_harmony_projects omitted until live-proven.',
    },
    gemini: {
      origin: 'https://gemini.google.com',
      loginUrl: 'https://gemini.google.com/app',
      deepLinkPattern: 'https://gemini.google.com/app/{id}',
      selectors: {
        signIn:
          'a[href*="accounts.google.com"], button[aria-label*="Sign in" i], a[aria-label*="Sign in" i]',
        signInToSaveActivity: 'button, a, p, span, div',
        accountChip:
          'img[alt*="Google Account" i], button[aria-label*="Google Account" i], a[aria-label*="Google Account" i]',
        historyItem: 'a[href*="/app/"]',
        historyScrollContainer: '',
        emptyHistoryState: '',
      },
      endpoints: {},
      notes:
        "DOM-first per S4: no stable first-party history search endpoint at desk. Title-match on history rail links (/app/{id}). Prefill none — cascade deep link → https://gemini.google.com/app. Auth (S5): login_required when Sign in CTAs AND 'Sign in to save activity' are both visible without items/chip (full shell wins over a rail-only / empty-history heuristic). One-signal alone → unavailable. Owner signals: history items or account chip, or proven history rail without the full sign-in upsell (rail alone can reach empty without a chip). Soft reach ceiling: HISTORY_SCROLL_MAX_ROUNDS (~8) / HISTORY_ITEM_SOFT_CAP (~120) still authorize empty by design when the history rail (or empty-history state) is proven and the scanned window has no title match. Incomplete: budget exhausted mid-scan → timeout (history_budget_exhausted); rail never proven → unavailable (history_rail_missing) — never empty. Account chip alone does not prove the rail. Selectors stub-level pending live polish (BL-025). Gems/folders: any visible /app/ history links are collected; no invented Projects clone.",
    },
  },
};
