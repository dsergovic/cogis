...
export default {
  ...
  platforms: {
    ...
    gemini: {
      ...
      endpoints: {},
      notes:
        'DOM-first per S4: no stable first-party history search endpoint at desk. Title-match on history rail links (/app/{id}). Prefill none — cascade deep link → https://gemini.google.com/app. Soft reach ceiling: HISTORY_SCROLL_MAX_ROUNDS (~8) / HISTORY_ITEM_SOFT_CAP (~120) still authorize empty by design when the scanned window has no title match (unread older history is a platform limit). Incomplete scroll (budget exhausted mid-scan) → timeout/unavailable, not empty. Selectors are stub-level pending live polish (highest churn platform; M5 pack). Gems/folders: any visible /app/ history links are collected; no invented Projects clone.',
    },
  },
};
