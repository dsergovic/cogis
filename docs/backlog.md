...
| ID | Status | Item | Notes |
| --- | --- | --- | --- |
| BL-035 | open | **Collapse Cogis-created lab tabs into one tab group** | `ensurePlatformTab` already creates with `active: false` and `launchSearch` closes Cogis-created tabs in its `finally` (adopted user tabs are never closed — BL-001), but a four-lab fan-out still puts up to four tabs in the strip for the life of the search. `chrome.tabs.group()` + `chrome.tabGroups.update({ collapsed: true })` would render them as one collapsed chip instead. Chrome has no hidden-tab API (`tabs.hide()` is Firefox-only), so this is the honest ceiling; it also keeps what Cogis opened visible to the user rather than concealing it. Costs a new `tabGroups` permission on the Web Store listing — weigh against BL-004 / trader-declaration work before adding permissions. Group only tabs Cogis created; never regroup a user's own tab. Raised during the M8c live smoke, 2026-07-31. |
...
| 2026-07-31 | BL-035 collapsed tab group for Cogis-created lab tabs (M8c smoke; tab-strip noise). |
